...
} from '@/components/messenger/recipient-details-modal';
import {
  ...
  const load = useCallback(
    async ({ resumeDrafts = false, silent = false } = {}) => {
      ...
      try {
        ...
        setFeed(
          await Promise.all(items.map((item) => decryptFeedItem(item, keys))),
        );
      } catch {
        if (!silent)
          setError('Broadcast could not be loaded. Please try again.');
      } finally {
  ...
  return (
    <>
      <div className="grid h-full min-h-0 min-w-0 grid-rows-[76px_minmax(0,1fr)_auto] bg-ice">
        ...
        <main
          className="min-h-0 min-w-0 max-w-full overflow-x-hidden overflow-y-auto bg-ice px-7 py-6 max-sm:px-3"
          aria-live="polite"
        >
          <div className="mx-auto w-full min-w-0 max-w-4xl">
            ...
            {error && (
              <div
                className="mb-4 flex items-center gap-2 rounded-xl border border-error/20 bg-error-bg px-4 py-3 text-xs text-error"
                role="alert"
              >
                <AlertCircle size={16} />{' '}
            ...
            {!chronologicalFeed ? (
              <div
                className="grid min-h-56 place-items-center text-secondary"
                role="status"
              >
              ...
              <div className="grid min-h-56 place-items-center text-center">
                <div>
                  ...
                  <h2 className="mt-3 text-base font-semibold">
                    Your Broadcast channel is ready
                  </h2>
                    ...
                    <div className="grid min-w-0 gap-2" key={item.id}>
                      ...
                      {own && (
                        <article
                          ...
                          aria-label="BeSeen delivery confirmation"
                        >
                          <div className="min-w-0 max-w-[min(62%,560px)] rounded-2xl rounded-bl-md bg-white px-4 py-3 max-sm:max-w-[90%]">
                            ...
                            <p className="mt-2 text-sm text-secondary">
                              Your broadcast was sent to{' '}
                              {item.manifest.audienceCount.toLocaleString()}{' '}
                              {item.manifest.audienceCount === 1
                                ? 'person'
                                : 'people'}
                              .
                            </p>
        ...
        <footer className="min-w-0 max-w-full overflow-x-hidden border-t border-border bg-white p-4">
          <form
            className="mx-auto grid min-w-0 max-w-4xl grid-cols-[minmax(0,1fr)_52px] items-end gap-3"
            onSubmit={publish}
          >
            <label className="relative block min-w-0">
              ...
              <textarea
                ref={input}
                className="block min-h-12 max-h-36 w-full resize-none rounded-xl border border-border bg-white px-4 py-3 text-sm leading-6 outline-none placeholder:text-muted focus:border-brand"
            ...
            <button
              className="grid size-13 cursor-pointer place-items-center rounded-xl border-0 bg-brand text-white transition hover:bg-[#0c3bd6] disabled:cursor-not-allowed disabled:opacity-45"
              disabled={
                sending || !draft.trim() || draftBytes > MAX_BROADCAST_BYTES
              }

...
function MessengerWorkspace({ user, keys }: { user: User; keys: DerivedKeys }) {
  ...
  const [replyTarget, setReplyTarget] =
    useState<DecryptedMessengerMessage | null>(null);
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      .then(async ([conversation, loadedContext, history, pending]) => {
        const decrypted = await decryptItems(
          history.items,
          keys,
          cache.current,
        );
  ...
  async function loadMoreConversations() {
    ...
    try {
      const page = await messengerApi.listConversations({
        limit: 20,
        cursor: nextCursor,
      });
      setConversations((current) => {
  ...
  const afterSuccessfulSend = useCallback(
    async (
      message: Awaited<
        ReturnType<typeof retryPendingMessengerMessage>
      >['message'],
    ) => {
      ...
      if (message.unlockedBounty) {
        setMessages((current) =>
          applyUnlockedBounty(current, message.unlockedBounty!),
        );
        toast(
          'Reward unlocked',
          'The other person can now collect this demo reward.',
        );
      }
      if (activeConversationId) {
  ...
  async function sendMessage(event: FormEvent) {
    ...
    if (!activeConversationId || sending || hasPendingRetry || !draft.trim())
      return;
    setSending(true);
    setSendError(null);
    setSendErrorCode(null);
    ...
    try {
      ...
      setSendErrorCode(
        cause instanceof ApiError ? cause.code : 'NETWORK_UNKNOWN',
      );
    } finally {
      setSending(false);
    }
  ...
  const otherParticipant =
    context?.otherParticipant ?? activeConversation?.otherParticipant ?? null;
  ...
  return (
    ...
    >
      <aside
        className={cn(
          'flex min-h-0 min-w-0 flex-col overflow-hidden border-r border-border bg-subtle',
      ...
      <div
        className={cn(
          'min-h-0 min-w-0 max-w-full overflow-hidden bg-ice',
          !broadcastOpen && !activeConversationId && 'max-[720px]:hidden',
        )}
      >
        {broadcastOpen ? (
          ...
          <motion.div
            className="grid h-full min-h-0 min-w-0 grid-rows-[76px_minmax(0,1fr)_auto] bg-ice"
          ...
          >
            <header className="flex min-w-0 items-center gap-3 border-b border-border bg-white px-5">
              ...
              <button
                ...
                onClick={() =>
                  otherParticipant &&
                  setProfileUsername(otherParticipant.username)
                }
                type="button"
              >
                ...
                <p className="mt-1 flex items-center gap-1.5 text-[11px] text-muted">
                  <ShieldCheck className="text-brand" size={13} /> Encrypted in
                  browser
                </p>
              </button>
            </header>
            ...
            <main
              className="min-h-0 min-w-0 max-w-full overflow-x-hidden overflow-y-auto bg-[#FEF7EF] px-6 py-5 max-sm:px-3"
              aria-live="polite"
            >
              {hasMoreMessages && (
                ...
                <div className="mx-auto grid w-full min-w-0 max-w-[860px] gap-3">
                  {timelineItems.map((item) => {
                        ...
                        >
                          <div className="min-w-0 max-w-[min(62%,560px)] overflow-hidden rounded-2xl rounded-bl-md bg-info-bg text-navy max-sm:max-w-[90%]">
                            <div className="flex min-w-0 items-center gap-2 border-b border-lilac bg-white px-4 py-3">
                              ...
                              >
                                {timeLabel(broadcast.publishedAt)}
                              </time>
                            </div>
                            <div className="px-4 py-3.5">
                              {broadcast.state === 'decrypted' ? (
                                ...
                                <p className="flex items-center gap-2 text-sm text-muted">
                                  <AlertCircle size={16} /> Unable to decrypt
                                  this broadcast
                                </p>
                              )}
                            </div>
                    ...
                    const reply = message.manifest.replyToMessageId
                      ...
                      : null;
                    const beneficiary =
                      message.manifest.recipientId === user.id;
                    return (
                      <div className="contents" key={message.id}>
                        {message.id === unreadMessageId && unreadMarker && (
                          <motion.div
                            ...
                            role="separator"
                            aria-label={`${unreadMarker.count} unread ${unreadMarker.count === 1 ? 'message' : 'messages'}`}
                          >
                        ...
                        >
                          <div
                            className={cn(
                              'min-w-0 max-w-[min(62%,560px)] overflow-hidden rounded-2xl px-4 py-3 max-sm:max-w-[90%]',
                              outgoing
                                ? 'rounded-br-md bg-info-bg text-navy'
                              ...
                              <div className="mt-3 max-w-full overflow-hidden rounded-2xl border border-border bg-white text-navy">
                                <div className="flex min-w-0 items-center gap-2.5 bg-lime/45 px-3.5 py-3">
                                  ...
                                  <strong className="min-w-0 flex-1 text-xs">
                                    Demo bounty · {message.bounty.amount}{' '}
                                    {message.bounty.assetCode}
                                  </strong>
                                  <BountyBadge bounty={message.bounty} />
                                </div>
                                <div className="px-3.5 py-2.5">
                                  <p className="text-[11px] leading-4 text-secondary">
                                    This is demo metadata only.
                                  </p>
                                  <p className="mt-1 text-[10px] text-muted">
                                    No real payment or escrow will be made.
                                  </p>
                                </div>
                                {beneficiary &&
                                  message.bounty.status === 'claimable' && (
                                    <button
                                      ...
                                      disabled={
                                        claimingBountyId === message.bounty.id
                                      }
                                      onClick={() =>
                                        void claimBounty(message.bounty!)
                                      }
                                      type="button"
                                    >
                                      {claimingBountyId === message.bounty.id
            ...
            <footer className="min-w-0 max-w-full overflow-x-hidden border-t border-border bg-white px-4 py-2">
              ...
              {hasPendingRetry && (
                <div
                  className="mb-3 flex flex-wrap items-center gap-3 rounded-xl border border-warning/25 bg-warning-bg px-3 py-2 text-xs text-warning"
                  role="alert"
                >
                  <RotateCcw size={16} />
                  <span className="min-w-0 flex-1">
                    {sendError ??
                      'We could not confirm whether this message was sent.'}
                  </span>
                  {sendErrorCode !== 'MESSAGE_ID_CONFLICT' && (
                    <button
                ...
                {replyTarget && (
                  ...
                  >
                    ...
                    <span className="min-w-0 flex-1">
                      ...
                      <span className="mt-0.5 block truncate">
                        {replyTarget.state === 'decrypted'
                          ? replyTarget.plaintext
                          : 'Message unavailable'}
                      </span>
                    </span>
                    <button
                      className="grid size-8 shrink-0 cursor-pointer place-items-center rounded-lg text-muted transition hover:bg-white hover:text-navy"
              ...
              <form
                className="mx-auto grid w-full max-w-[860px] grid-cols-[44px_minmax(0,1fr)_auto_auto] items-center gap-2 rounded-2xl border border-border bg-white p-2 transition focus-within:border-brand/35 max-sm:grid-cols-[40px_minmax(0,1fr)_auto]"
                  ...
                  {showBounty ? (
                    ...
                    >
                      <label className="relative flex min-h-11 w-29 shrink-0 items-center rounded-xl border border-border bg-white pl-9 pr-7 text-xs font-semibold text-navy transition focus-within:border-brand/35">
                        <CircleDollarSign
                          className="absolute left-2.5 text-brand"
                          size={20}
                          aria-hidden="true"
                        />
                        ...
                        <select
                          ...
                          value={bountyAsset}
                          onChange={(event) =>
                            setBountyAsset(event.target.value)
                          }
                      ...
                      <label className="relative flex min-h-11 w-21 shrink-0 items-center rounded-xl border border-border bg-white px-3 pr-7 text-xs font-semibold text-navy transition focus-within:border-brand/35">
                        ...
                        <select
                          ...
                          onChange={(event) =>
                            setBountyDuration(event.target.value)
                          }
                          aria-label="Time to reply"
                        >
                      ...
                      <button
                        className="grid size-11 shrink-0 cursor-pointer place-items-center rounded-xl border border-brand/20 bg-info-bg text-brand transition hover:border-brand/40"

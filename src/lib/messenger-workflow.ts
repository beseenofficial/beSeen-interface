...
async function deliverAttempt(attempt: PendingMessengerAttempt): Promise<{
  ...
  // Every failure keeps the encrypted retry record. When a bounty was locked
  // on-chain before submission, the funds may already be reserved, so the
  // record (client message ID, contract bounty ID, signed manifest) must
  // survive for an explicit retry or an explicit discard by the user.
...
export async function createAndSendMessengerMessage(input: {
  ...
  plaintext: string;
  keys: DerivedKeys;
  /** Final idempotency key; generated by the caller before any on-chain work. */

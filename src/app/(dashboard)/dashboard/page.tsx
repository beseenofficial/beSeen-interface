...
function StatCard({ icon: Icon, iconClass, label, value, hint, onClick }: StatCardProps) {
  const content = <><span className={`grid size-11 place-items-center rounded-full ${iconClass}`}><Icon size={21} strokeWidth={1.8} /></span><span className="mt-5 block text-sm font-semibold">{label}</span><strong className="mt-1.5 block text-[27px] font-medium leading-none tracking-[-0.03em]">{value}</strong><span className="mt-2.5 block text-xs text-muted">{hint}</span></>;
  if (onClick) return <button className="min-h-46 cursor-pointer rounded-2xl border border-border bg-white p-5 text-left transition hover:border-brand/25 hover:shadow-elevated" onClick={onClick} type="button">{content}</button>;
  return <article className="min-h-46 rounded-2xl border border-border bg-white p-5">{content}</article>;
}
...
export default function OverviewPage() {
  ...
  return (
    <div className="w-full px-10 pb-10 pt-9 max-[1100px]:px-5 max-sm:px-4 max-sm:pb-8 max-sm:pt-6">
      <header>
        <h1 className="text-[32px] font-semibold tracking-[-0.04em] max-sm:text-[28px]">Overview</h1>
      </header>

      <section className="mt-4 grid gap-4 xl:grid-cols-4 xl:grid-rows-[68px_184px]" aria-label="Bounty and earnings summary">
        <article className="relative min-h-[268px] overflow-hidden rounded-2xl border border-border bg-[radial-gradient(circle_at_45%_42%,#ffffff_0%,#fbfcff_54%,#f1f5ff_100%)] p-6 xl:col-span-2 xl:row-span-2 xl:min-h-0">
          ...
          <div className="relative z-10 flex h-full items-start gap-5">
            <span className="grid size-14 shrink-0 place-items-center rounded-full bg-info-bg text-brand"><Gift size={27} strokeWidth={1.8} /></span>
            <div className="pt-2">
              <h2 className="text-[17px] font-semibold">Available bounties</h2>
              <strong className="mt-2 block text-[62px] font-medium leading-[0.95] tracking-[-0.05em] text-brand">$0</strong>
              <p className="mt-4 text-sm text-secondary">No unanswered messages</p>
              <Link className="mt-7 inline-flex min-h-12 items-center gap-7 rounded-xl bg-brand px-5 text-sm font-semibold text-white transition hover:bg-[#0c3bd6]" href="/dashboard/messenger">Go to Messenger <ArrowRight size={19} /></Link>
            </div>
          </div>
        </article>
      ...
      <section className="mt-4 grid gap-4 sm:grid-cols-2 xl:grid-cols-4" aria-label="Aura and wallet summary">
